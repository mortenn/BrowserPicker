...
{
	public BrowserProfileViewModel(BrowserProfile model, BrowserViewModel parentBrowser)
		: base(model)
	{
	...
	public DelegateCommand Select =>
		select ??= new DelegateCommand(() => ParentBrowser.LaunchWithProfile(false, Model));

	/// <summary>
	/// Launches the parent browser with this profile in privacy mode.
	/// </summary>
	public DelegateCommand SelectPrivacy =>
		select_privacy ??= new DelegateCommand(() => ParentBrowser.LaunchWithProfile(true, Model));
